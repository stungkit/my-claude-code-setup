...
| **ai-image-creator** | Generate PNG images using AI (Gemini, FLUX.2, Riverflow, SeedDream, GPT-5) with configurable aspect ratios and resolutions. Supports transparent backgrounds (`-t`), reference image editing (`-r`), cost tracking (`--costs`), prompt enhancement patterns, and composite banner generation for multi-size logo banners. Requires [setup](.claude/skills/ai-image-creator/references/setup-guide.md) |
